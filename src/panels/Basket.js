...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  const remember = [localStorage.getItem('setFaster') === 'true', localStorage.getItem('setTime'), localStorage.getItem('setSelfService') === 'true' ];
  const [ faster, setFaster ] = useState(remember[0]);
    localStorage.setItem('setFaster', faster);
  const [ time, setTime ] = useState(remember[1]);
    localStorage.setItem('setTime', time);
  const [ selfService, setSelfService ] = useState(remember[2]);
    localStorage.setItem('setSelfService', selfService);
